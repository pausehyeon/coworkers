...
(function(window, Calendar) {
    ...
    $.ajax({
    	url: Coworkers.domain + '/meeting-rooms',
    	method: 'GET',
    	dataType: 'json'
    }).done(function(arr){
    	var colors = ['#9e5fff', '#00a9ff', '#ff5583', '#03bd9e', '#bbdc00', '#9d9d9d', '#ffbb3b', '#ff4040'];
    	arr.forEach(function(item, index) {
      	tempCal = new CalendarInfo();
        tempCal.id = item.rid;
        tempCal.name = item.name;
        tempCal.color = '#ffffff';
        tempCal.bgColor = colors[index%colors.length]
        tempCal.dragBgColor = colors[index%colors.length];
        tempCal.borderColor = colors[index%colors.length];
        addCalendar(tempCal);
    	});
